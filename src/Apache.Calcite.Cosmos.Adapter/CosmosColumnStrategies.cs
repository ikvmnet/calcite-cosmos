...
namespace Apache.Calcite.Cosmos.Adapter
{
    ...
    class CosmosColumnStrategies : NullInitializerExpressionFactory
    {
        ...
        public override ColumnStrategy generationStrategy(RelOptTable table, int iColumn)
        {
            ...
            // The JSON column, last, and the same document as the map column. DEFAULT, like the map
            // column: it may be read, may be supplied, and may be omitted. Not VIRTUAL, which would
            // make a scan project null in its place, and no longer STORED, which is what _ts and
            // _etag are and which would forbid supplying it — an insert or an update writes a whole
            // document through either document column.
