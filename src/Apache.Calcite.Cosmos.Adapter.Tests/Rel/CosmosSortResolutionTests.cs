...
namespace Apache.Calcite.Cosmos.Adapter.Tests.Rel
{
    ...
    public class CosmosSortResolutionTests
    {
        ...
        bool Resolves(RelCollation collation, params int[] nonNullFields) =>
            CosmosSort.TryResolveSortKeys(collation, Fields, RowType(), "c", nonNullFields, null, null, out _, out _);
        ...
        public void AMultiKeySortNeedsTheGuaranteeOnEveryNullableKey()
        {
            ...
            CosmosSort.TryResolveSortKeys(RelCollations.of(partial), Fields, RowType(), "c", new[] { 1 }, null, null, out _, out _).Should().BeFalse();
            CosmosSort.TryResolveSortKeys(RelCollations.of(partial), Fields, RowType(), "c", new[] { 1, 2 }, null, null, out _, out _).Should().BeTrue();
        }
