...
namespace Apache.Calcite.Cosmos.Adapter.Tests.Metadata
{
    ...
    /// <summary>
    /// Which patterns say a stored string spells a UUID, and which spelling it spells it in.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The shape is decided rather than looked up, so these rows are about the <em>argument</em>
    /// rather than about a table: a confinement nobody enumerated, an alternation read as a union, a
    /// range read over code points. Each was a wrong answer or a missing one while the spellings were
    /// tabulated.
    /// </para>
    /// <para>
    /// Recognition and rendering are asked separately throughout, because they are separate
    /// questions: what a pattern proves about every string it admits, and how a literal is written
    /// into the container so a comparison can be pushed.
    /// </para>
    /// </remarks>
    public class CosmosUuidFormsTests
    {
        ...
        public void AConfinementNobodyEnumeratedIsStillACanonicalUuid()
        {
            ...
            })
            {
                ...
                form.Should().Be(CosmosUuidForms.CanonicalLower, "for " + pattern);
            }
        }
        ...
        public void PinningMoreThanTheFirstDigitIsStillConfined()
        {
            foreach (var pattern in new[]
            {
                ...
                CosmosStoredForms.Recognise(pattern).Should().Be(CosmosUuidForms.CanonicalLowerSortable, "for " + pattern);
        }
        ...
        public void AnAlternationIsTheUnionOfItsBranches()
        {
            ...
            CosmosStoredForms.Recognise($"^{Body}$")
                .Should().Be(CosmosUuidForms.CanonicalLowerSortable, "the branch on its own is confined");
            ...
            foreach (var pattern in new[]
            {
                ...
                CosmosStoredForms.Recognise(pattern).Should().Be(CosmosUuidForms.CanonicalLower,
        ...
        public void TheBraceAndTheHyphenAreAxesOfTheShapeAndCostNoRelation()
        {
            ...
            foreach (var (pattern, name, stored) in shapes)
            {
                ...
                CosmosUuidForms.IsUuid(form.Value).Should().BeTrue("the reader takes every one of these, for " + pattern);
        ...
        public void AnOutOfClassLiteralIsRenderedUnderTheUnsignedComparison()
        {
            ...
            confined.Should().Be(CosmosUuidForms.CanonicalLowerSortable);
