        ...
        static readonly PlannerQuery[] Corpus = new[]
        {
            ...
            new PlannerQuery(
                ...
                WHERE c."$.category" = 'bikes'
                  ...
                  AND JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) BETWEEN 10.0 AND 500.0
                  AND JSON_VALUE(c."DOC", '$.stock' RETURNING INTEGER) > 0
                  AND JSON_VALUE(c."DOC", '$.brand') <> 'unbranded'
                  AND JSON_VALUE(c."DOC", '$.colour') IN ('red', 'blue', 'green')
                  AND IS_DEFINED(JSON_VALUE(c."DOC", '$.warranty'))
                  AND NOT IS_NULL(JSON_VALUE(c."DOC", '$.sku'))
                """,
            ...
            new PlannerQuery(
                ...
                """
                SELECT * FROM products AS c
                WHERE (c."$.category" = 'bikes' AND JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) < 100.0)
                   OR (c."$.category" = 'shoes' AND JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) < 60.0)
                   OR (c."$.category" = 'tents' AND JSON_VALUE(c."DOC", '$.stock' RETURNING INTEGER) > 20)
                   OR (c."$.category" = 'packs' AND IS_DEFINED(JSON_VALUE(c."DOC", '$.clearance')))
                """,
            ...
            new PlannerQuery(
                ...
                """
                SELECT * FROM products AS c
                WHERE IS_DEFINED(JSON_VALUE(c."DOC", '$.dimensions'))
                  AND IS_OBJECT(JSON_VALUE(c."DOC", '$.dimensions'))
                  AND IS_ARRAY(JSON_VALUE(c."DOC", '$.tags'))
                  AND IS_NUMBER(JSON_VALUE(c."DOC", '$.price'))
                  AND IS_STRING(JSON_VALUE(c."DOC", '$.sku'))
                  AND NOT IS_NULL(JSON_VALUE(c."DOC", '$.brand'))
                """,
            ...
            new PlannerQuery(
                "Filter.RegexAndLike",
                PlannerQueryCategory.Filter,
                """SELECT * FROM products AS c WHERE REGEXMATCH(c."id", '^a[0-9]+$') AND c."$.category" LIKE 'bi%' AND JSON_VALUE(c."DOC", '$.sku') NOT LIKE '%-old'""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Filter,
                """SELECT * FROM shipments AS c WHERE JSON_VALUE(c."DOC", '$.shipment.region') = 'eu-west' AND JSON_VALUE(c."DOC", '$.weight' RETURNING DOUBLE) > 2.5""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Filter,
                """SELECT * FROM archive AS c WHERE JSON_VALUE(c."DOC", '$.reason') = 'expired' AND c."$.category" = 'bikes'""",
            ...
            new PlannerQuery(
                ...
                """
                SELECT * FROM products AS c
                WHERE JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) > 10.0
                  AND JSON_VALUE(c."DOC", '$.stock' RETURNING INTEGER) < 500
                  AND JSON_VALUE(c."DOC", '$.label') = 'sale'
                  AND CAST(JSON_VALUE(c."DOC", '$.listed') AS DATE) > DATE '2020-01-01'
                  AND CAST(JSON_VALUE(c."DOC", '$.active') AS BOOLEAN)
                """,
            ...
            new PlannerQuery(
                ...
                """SELECT JSON_VALUE(c."DOC", '$.sku') AS "sku", JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) AS "price", JSON_VALUE(c."DOC", '$.stock' RETURNING INTEGER) AS "stock", c."$.category" AS "cat" FROM products AS c""",
                "What a view over a container is made of: four casts giving four document paths a relational type."),

            new PlannerQuery(
                "Project.DeepPaths",
                PlannerQueryCategory.Projection,
                """SELECT JSON_VALUE(c."DOC", '$.a.b.c.d') AS "deep", JSON_VALUE(c."DOC", '$.tags[0]') AS "first", JSON_VALUE(c."DOC", '$.metadata.sku') AS "sku", JSON_VALUE(c."DOC", c."id") AS "dynamic" FROM products AS c""",
                "Four ways of naming a path — nesting, indexing, both, and a key not known until the row is read."),

            new PlannerQuery(
                "Project.OverFilterAndSort",
                PlannerQueryCategory.Projection,
                """SELECT JSON_VALUE(c."DOC", '$.sku') AS "sku" FROM products AS c WHERE c."$.category" = 'bikes' ORDER BY c."_ts" DESC FETCH NEXT 25 ROWS ONLY""",
            ...
            new PlannerQuery(
                "Project.Arithmetic",
                PlannerQueryCategory.Projection,
                """SELECT c."id", JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) * 1.2 + 5.0 AS "gross", UPPER(JSON_VALUE(c."DOC", '$.sku')) AS "sku" FROM products AS c WHERE c."$.category" = 'bikes'""",
            ...
            new PlannerQuery(
                "Aggregate.Rollup",
                PlannerQueryCategory.Aggregate,
                """SELECT c."$.category", SUM(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "s", MAX(c."_ts") AS "m" FROM products AS c GROUP BY ROLLUP(c."$.category")""",
            ...
            new PlannerQuery(
                "Aggregate.Average",
                PlannerQueryCategory.Aggregate,
                """SELECT c."$.category", AVG(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "a" FROM products AS c GROUP BY c."$.category" """,
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Aggregate,
                """SELECT c."$.category", AVG(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "a" FROM products AS c GROUP BY ROLLUP(c."$.category")""",
            ...
            new PlannerQuery(
                ...
                SELECT c."$.category",
                       COUNT(*) AS "n",
                       SUM(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "total",
                       MIN(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "cheapest",
                       MAX(JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE)) AS "dearest",
                       AVG(JSON_VALUE(c."DOC", '$.stock' RETURNING INTEGER)) AS "stock",
                       COUNT(JSON_VALUE(c."DOC", '$.clearance')) AS "clearance"
                FROM products AS c
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Sort,
                """SELECT * FROM (SELECT JSON_VALUE(c."DOC", '$.sku') AS "sku", c."id" AS "ident" FROM products AS c) AS t ORDER BY t."ident" FETCH NEXT 10 ROWS ONLY""",
            ...
            new PlannerQuery(
                "Sort.ByCastColumn",
                PlannerQueryCategory.Sort,
                """SELECT * FROM (SELECT JSON_VALUE(c."DOC", '$.sku') AS "sku", c."id" AS "ident" FROM products AS c) AS t ORDER BY t."sku" FETCH NEXT 10 ROWS ONLY""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Unnest,
                """SELECT c."id" FROM products AS c, UNNEST(StringToArray(JSON_QUERY(c."DOC", '$.tags'))) AS t WHERE CAST(t AS VARCHAR) = 'steel'""",
            ...
            new PlannerQuery(
                "Unnest.TwoArrays",
                PlannerQueryCategory.Unnest,
                """SELECT c."id" FROM products AS c, UNNEST(StringToArray(JSON_QUERY(c."DOC", '$.tags'))) AS t, UNNEST(StringToArray(JSON_QUERY(c."DOC", '$.sizes'))) AS s WHERE CAST(t AS VARCHAR) = 'steel' AND CAST(s AS VARCHAR) = 'L'""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Unnest,
                """SELECT c."id" FROM (SELECT * FROM products AS p ORDER BY p."_ts" FETCH NEXT 50 ROWS ONLY) AS c, UNNEST(StringToArray(JSON_QUERY(c."DOC", '$.tags'))) AS t""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Unnest,
                """SELECT CAST(t AS VARCHAR) AS "tag", COUNT(*) AS "n" FROM products AS c, UNNEST(StringToArray(JSON_QUERY(c."DOC", '$.tags'))) AS t WHERE c."$.category" = 'bikes' GROUP BY CAST(t AS VARCHAR)""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Search,
                """SELECT c."id" FROM products AS c WHERE FULLTEXTCONTAINSALL(JSON_VALUE(c."DOC", '$.description'), 'steel', 'frame', 'road', 'touring')""",
                "A variadic full text predicate, whose operand count the rule has to check against the service's limit."),

            new PlannerQuery(
                "Search.RankedByScore",
                PlannerQueryCategory.Search,
                """SELECT c."id" FROM products AS c ORDER BY FULLTEXTSCORE(JSON_VALUE(c."DOC", '$.name'), 'steel') FETCH FIRST 10 ROWS ONLY""",
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Search,
                """SELECT c."id" FROM products AS c ORDER BY RRF(FULLTEXTSCORE(JSON_VALUE(c."DOC", '$.name'), 'steel'), FULLTEXTSCORE(JSON_VALUE(c."DOC", '$.tags'), 'frame')) FETCH FIRST 10 ROWS ONLY""",
            ...
            new PlannerQuery(
                ...
                WHERE c."$.category" = 'bikes' AND FULLTEXTCONTAINS(JSON_VALUE(c."DOC", '$.description'), 'steel')
                ORDER BY RRF(FULLTEXTSCORE(JSON_VALUE(c."DOC", '$.name'), 'steel'), FULLTEXTSCORE(JSON_VALUE(c."DOC", '$.description'), 'frame'))
            ...
            new PlannerQuery(
                ...
                """SELECT o."id", c."DOC" FROM orders AS o JOIN customers AS c ON o."$.customer" = c."id" WHERE o."$.customer" = 'acme'""",
            ...
            new PlannerQuery(
                "Join.LeftOuter",
                PlannerQueryCategory.Join,
                """SELECT o."id", c."DOC" FROM orders AS o LEFT JOIN customers AS c ON o."$.customer" = c."id" """,
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Join,
                """
                SELECT c."id", COUNT(*) AS "n", SUM(JSON_VALUE(o."DOC", '$.total' RETURNING DOUBLE)) AS "spend"
            ...
            new PlannerQuery(
                ...
                SELECT t."id", t."cat" FROM (
                    SELECT c."id" AS "id", c."$.category" AS "cat",
                           RANK() OVER (PARTITION BY c."$.category" ORDER BY JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) DESC) AS "r"
            ...
            new PlannerQuery(
                "Window.RunningTotal",
                PlannerQueryCategory.Window,
                """SELECT c."id", SUM(JSON_VALUE(c."DOC", '$.total' RETURNING DOUBLE)) OVER (PARTITION BY c."$.customer" ORDER BY c."_ts" ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "running" FROM orders AS c WHERE c."$.customer" = 'acme'""",
            ...
            new PlannerQuery(
                ...
                """INSERT INTO archive ("DOC") VALUES ('{"id":"x","category":"bikes"}')""",
                "A write of one row, which never enters the convention: Cosmos SQL has no DML, so this is item CRUD over a row source."),

            new PlannerQuery(
                "Dml.InsertSelect",
                PlannerQueryCategory.Dml,
                """INSERT INTO archive ("DOC") SELECT p."DOC" FROM products AS p WHERE p."$.category" = 'bikes' """,
            ...
            new PlannerQuery(
                "Dml.Update",
                PlannerQueryCategory.Dml,
                """UPDATE products SET "DOC" = "DOC" WHERE "$.category" = 'bikes' AND "id" = 'y' """,
            ...
            new PlannerQuery(
                ...
                FROM (
                    SELECT p."$.category" AS "cat",
                           ...
                           COUNT(*) AS "n",
                           SUM(JSON_VALUE(o."DOC", '$.total' RETURNING DOUBLE)) AS "spend"
                    FROM products AS p, UNNEST(StringToArray(JSON_QUERY(p."DOC", '$.tags'))) AS g
                    JOIN orders AS o ON o."id" = p."id"
                    WHERE p."$.category" IN ('bikes', 'shoes')
                      AND JSON_VALUE(p."DOC", '$.price' RETURNING DOUBLE) > 20.0
                ...
                FROM (
                    ...
                    FROM (
                        ...
                        WHERE e."$.tenant" IN ('acme', 'globex')
                          ...
                          AND IS_DEFINED(JSON_VALUE(e."DOC", '$.step'))
                          AND JSON_VALUE(e."DOC", '$.step') <> 'abandoned'
            ...
            new PlannerQuery(
                ...
                PlannerQueryCategory.Composite,
                """
                SELECT p."id" AS "id", c."DOC" AS "customer"
                FROM (
                    SELECT x."id" AS "id", x."$.category" AS "category"
                    FROM products AS x
                    WHERE x."$.category" = 'bikes'
                      AND FULLTEXTCONTAINSALL(JSON_VALUE(x."DOC", '$.description'), 'steel', 'frame')
                      AND VECTORDISTANCE(JSON_VALUE(x."DOC", '$.embedding'), JSON_VALUE(x."DOC", '$.query')) < 0.4
                    ORDER BY RRF(FULLTEXTSCORE(JSON_VALUE(x."DOC", '$.name'), 'steel'), FULLTEXTSCORE(JSON_VALUE(x."DOC", '$.tags'), 'frame'))
            ...
            new PlannerQuery(
                ...
                SELECT c."id" AS "id", c."$.category" AS "cat"
                FROM products AS c
                WHERE (c."$.category" = 'bikes' AND JSON_VALUE(c."DOC", '$.price' RETURNING DOUBLE) BETWEEN 100.0 AND 500.0)
                   OR (c."$.category" = 'shoes' AND JSON_VALUE(c."DOC", '$.size' RETURNING INTEGER) IN (40, 41, 42, 43))
                   OR (c."$.category" = 'tents' AND FULLTEXTCONTAINS(JSON_VALUE(c."DOC", '$.description'), 'ultralight'))
                   OR (c."$.category" = 'packs' AND IS_DEFINED(JSON_VALUE(c."DOC", '$.clearance')) AND NOT IS_NULL(JSON_VALUE(c."DOC", '$.clearance')))
