...
than once and is recorded in section 7. The row model is the map column and the `_JSON` column, and
...
it is out of reach says more about the surface than the original argument did. See section 7.
...
- **`DISTINCT` with `ORDER BY` reaches promoted columns and not the map column** — *small, and what
  is left of it waits on section 7.* The null-placement rule refuses a nullable sort key, and a
